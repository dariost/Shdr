...
(function() {
  ...
  Snippets = {
    'DefaultVertex': ['#version 300 es', '', 'precision highp float;', 'in vec3 position;', 'in vec3 normal;', 'uniform mat3 normalMatrix;', 'uniform mat4 modelViewMatrix;', 'uniform mat4 projectionMatrix;', 'out vec3 fNormal;', 'out vec3 fPosition;', '', 'void main()', '{', '    fNormal = normalize(normalMatrix * normal);', '    vec4 pos = modelViewMatrix * vec4(position, 1.0);', '    fPosition = pos.xyz;', '    gl_Position = projectionMatrix * pos;', '}'].join('\n'),
    'DefaultFragment': ['#version 300 es', '', 'precision highp float;', 'uniform float time;', 'uniform vec2 resolution;', 'in vec3 fPosition;', 'in vec3 fNormal;', 'out vec4 color;', '', 'void main()', '{', '    color = vec4(fNormal, 1.0);', '}'].join('\n'),
